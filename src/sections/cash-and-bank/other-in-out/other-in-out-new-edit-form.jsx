...
export default function OtherInOutNewEditForm({ currentOtherInOut }) {
  ...
  const NewSchema = Yup.object().shape({
    otherIncomeType: Yup.string().required('Charge Type is required'),
  ...
  const defaultValues = useMemo(
    () => ({
      otherIncomeType: currentOtherInOut?.otherIncomeType || '',
      category: currentOtherInOut?.category || '',
      date: currentOtherInOut?.date ? new Date(currentOtherInOut?.date) : new Date(),
      description: currentOtherInOut?.description || '',
      paymentMode: currentOtherInOut?.paymentDetails?.paymentMode || '',
      account: currentOtherInOut?.paymentDetails?.account || null,
      cashAmount: currentOtherInOut?.paymentDetails?.cashAmount || '',
      bankAmount: currentOtherInOut?.paymentDetails?.bankAmount || '',
      branchId: currentOtherInOut
        ? {
          label: currentOtherInOut?.branch?.name,
          value: currentOtherInOut?.branch?._id,
        }
        : null,
      paymentType: currentOtherInOut?.status || '',
    }),
    [currentOtherInOut],
  );
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const payload = {
        otherIncomeType: data.otherIncomeType,
      ...
      const baseUrl = `${import.meta.env.VITE_BASE_URL}/${user?.company}/other-in-out?branch=${selectedBranchId}`;

      const res = currentOtherInOut
        ? await axios.put(
          `${import.meta.env.VITE_BASE_URL}/${user?.company}/other-in-out/${currentOtherInOut._id}?branch=${selectedBranchId}`,
          payload,
      ...
      router.push(paths.dashboard.cashAndBank.otherInOut.list);
      enqueueSnackbar(res?.data.message);
      reset();
    } catch (error) {
      enqueueSnackbar(currentOtherInOut ? 'Failed to update charge In/Out' : error.response?.data?.message, {
        variant: 'error',
      });
      console.error(error);
    }
...
OtherInOutNewEditForm.propTypes = {
  currentOtherInOut: PropTypes.object,
};
