...
export default function OtherIncomeNewEditForm({ currentOtherIncome }) {
    ...
    () => ({
      ...
      branch: currentOtherIncome
        ? {
            ...
            value: currentOtherIncome?.branch?._id,
          }
        : null,
    }),
    [currentOtherIncome]
    ...
    try {
      if (currentOtherIncome) {
        const res = await axios.put(
          `${import.meta.env.VITE_BASE_URL}/${user?.company}/other-income/${currentOtherIncome._id}`,
          payload
        );
        router.push(paths.dashboard.cashAndBank.otherIncome.list);
        enqueueSnackbar(res?.data.message);
        reset();
      } else {
        const res = await axios.post(
          `${import.meta.env.VITE_BASE_URL}/${user?.company}/other-income`,
          payload
        );
        router.push(paths.dashboard.cashAndBank.otherIncome.list);
        enqueueSnackbar(res?.data.message);
      ...
      enqueueSnackbar(
        currentOtherIncome ? 'Failed To update other income' : error.response.data.message,
        {
          variant: 'error',
        }
      );
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={8}>
          <Card sx={{ p: 3 }}>
            ...
            {/*          <iframe*/}
            {/*            src={currentOtherIncome ? file : file.preview}*/}
            {/*            width="100%"*/}
            {/*            height="100%"*/}
            {/*            title="pdf-preview"*/}
            {/*          />*/}
          ...
          <Box xs={12} md={8} sx={{ display: 'flex', justifyContent: 'end', mt: 3 }}>
            ...
            <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
              {currentOtherIncome ? 'Save' : 'Submit'}
            </LoadingButton>
          </Box>
        </Grid>
      </Grid>
...
OtherIncomeNewEditForm.propTypes = {
  currentOtherIncome: PropTypes.object,
};
