...
export default function AreaCreteView({ setTab }) {
  ...
  const handleClick = () => {
    ...
    const updatedArea = [...area, inputVal.toUpperCase()];
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setInputVal('');
          setArea(updatedArea);
          enqueueSnackbar('Area added successfully', { variant: 'success' });
          setTab('Permission');
          mutate();
        }
      })
  ...
  const handleDelete = (name) => {
    const updatedArea = area.filter((a) => a !== name);
