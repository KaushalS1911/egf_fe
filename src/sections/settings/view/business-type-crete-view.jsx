...
export default function BusinessTypeCreteView({ setTab }) {
  ...
  const handleClick = () => {
    if (!inputVal) {
      enqueueSnackbar('Business type cannot be empty', { variant: 'warning' });
      return;
    }
  ...
  const handleDelete = (type) => {
    const updatedBusinessTypes = businessTypes.filter((bt) => bt !== type);
        ...
        <Grid item xs={12} md={8}>
          <Card>
            <Stack spacing={3} sx={{ p: 3 }}>
              ...
              >
                {businessTypes.length !== 0 && businessTypes.map((type, index) => (
                  <Grid
                    container
                    key={index}
                    sx={{
